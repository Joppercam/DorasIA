<<<<<<< HEAD
<x-guest-layout>
    <h1 class="text-2xl font-bold text-white text-center mb-6">{{ __('Iniciar sesión') }}</h1>

    <!-- Session Status -->
    @if(session('status'))
        <div class="mb-6 bg-gray-800 border border-gray-600 text-gray-300 px-4 py-3 rounded-md">
            {{ session('status') }}
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div>
            ...
            <x-text-input id="email" 
                class="block mt-1 w-full bg-gray-700 border-gray-600 text-white focus:border-red-500 focus:ring-red-500" 
                type="email" 
                name="email" 
                :value="old('email')" 
                required 
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox" class="rounded border-gray-600 bg-gray-700 text-red-600 focus:ring-red-500" name="remember">
        ...
        <div class="flex items-center justify-between mt-4">
            @if (Route::has('password.request'))
                <a class="text-sm text-red-500 hover:text-red-400 focus:outline-none focus:underline transition ease-in-out duration-150" href="{{ route('password.request') }}">
                    {{ __('¿Olvidaste tu contraseña?') }}
                </a>
            @endif

            <a href="{{ route('register') }}" class="text-sm text-red-500 hover:text-red-400 focus:outline-none focus:underline transition ease-in-out duration-150">
                {{ __('Crear cuenta') }}
            </a>
        </div>
    </form>
    ...
    <div class="mt-8">
        <div class="relative">
            ...
            <div class="relative flex justify-center text-sm">
                <span class="px-2 bg-gray-800 bg-opacity-70 text-gray-400">{{ __('O inicia sesión con') }}</span>
            </div>
        </div>
    ...
    <div class="mt-6 text-xs text-center text-gray-500">
        ...
        <a href="#" class="text-red-500 hover:text-red-400">{{ __('Contacta con soporte') }}</a>
    </div>
</x-guest-layout>
=======
@extends('layouts.app')
