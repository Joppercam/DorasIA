<x-app-layout>
    <x-slot name="title">{{ $genre->name }}</x-slot>
    
    <!-- Hero Banner mejorado para el género -->
    <div class="hero-banner relative overflow-hidden bg-gradient-to-b from-transparent to-dorasia-bg-dark" 
        ...
        <!-- Imagen de fondo (compilado dinámicamente de títulos del género) -->
        <div class="absolute inset-0 z-0">
            @php
                // Obtener una imagen de fondo de uno de los títulos populares del género
                $backgroundTitle = $titles->where('backdrop', '!=', '')
                                        ->whereNotNull('backdrop')
                                        ->sortByDesc('vote_average')
                                        ->first();
                
                $backdropPath = $backgroundTitle && $backgroundTitle->backdrop 
                                ? $backgroundTitle->backdrop 
                                : 'backdrops/placeholder.jpg';
            @endphp
            
            <img src="{{ asset($backdropPath) }}" alt="{{ $genre->name }}" class="hero-banner__image opacity-60">
        ...
        <div class="relative z-10 flex flex-col justify-end sm:justify-center h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
            <div class="hero-banner__content max-w-2xl">
                <div class="category-badge">{{ $genre->titles_count ?? $titles->count() }} {{ $titles->count() == 1 ? 'Título' : 'Títulos' }}</div>
                
                <h1 class="text-4xl md:text-6xl font-bold mb-3 sm:mb-4 tracking-tight">{{ $genre->name }}</h1>
                
                <!-- Estadísticas del género -->
                <div class="genre-stats mb-4">
                    <div class="genre-stats__item">
                        <span class="genre-stats__value">{{ $titles->where('type', 'movie')->count() }}</span>
                        <span class="genre-stats__label">Películas</span>
                    </div>
                    
                    <div class="genre-stats__item">
                        <span class="genre-stats__value">{{ $titles->where('type', 'series')->count() }}</span>
                        <span class="genre-stats__label">Series</span>
                    </div>
                    
                    @php
                        $avgRating = $titles->avg('vote_average');
                    @endphp
                    @if($avgRating)
                    <div class="genre-stats__item">
                        <span class="genre-stats__value flex items-center">
                        ...
                        <span class="genre-stats__label">Rating promedio</span>
                    </div>
                    @endif
                </div>
                ...
                @else
                    <p class="text-base sm:text-lg text-gray-300 mb-3 sm:mb-4 leading-relaxed">
                        Explora nuestra colección de {{ $genre->name }} con los mejores títulos asiáticos. Descubre películas y series que han cautivado a audiencias de todo el mundo.
                    </p>
                @endif
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="catalogPage()">
        @if($titles->count() > 0)
            <!-- Navegación de géneros relacionados -->
            <div class="mb-6">
                <h2 class="text-lg font-medium mb-3">Géneros relacionados</h2>
                <div class="related-genres">
                    @php
                        // Obtener géneros relacionados basados en títulos que comparten este género
                        $relatedGenreIds = \App\Models\Title::whereHas('genres', function($query) use ($genre) {
                    ...
                    <a href="{{ route('catalog.genre', $genre->slug) }}" class="related-genres__item related-genres__item--active">
                        {{ $genre->name }}
                    </a>
                    
                    @foreach($relatedGenreIds as $relatedGenre)
                        <a href="{{ route('catalog.genre', $relatedGenre->slug) }}" class="related-genres__item">
                            {{ $relatedGenre->name }}
                        </a>
                    @endforeach
                </div>
            ...
            <div 
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <!-- Filtro por categoría (países) -->
                    <div>
                        <h3 class="text-sm font-medium mb-2">País/Origen</h3>
                        <div class="flex flex-wrap gap-2">
                            <button class="px-2 py-1 text-xs bg-gray-800 hover:bg-gray-700 rounded transition-colors" @click="applyFilter('country', 'kr')">Corea</button>
                            <button class="px-2 py-1 text-xs bg-gray-800 hover:bg-gray-700 rounded transition-colors" @click="applyFilter('country', 'jp')">Japón</button>
                            <button class="px-2 py-1 text-xs bg-gray-800 hover:bg-gray-700 rounded transition-colors" @click="applyFilter('country', 'cn')">China</button>
                            <button class="px-2 py-1 text-xs bg-gray-800 hover:bg-gray-700 rounded transition-colors" @click="applyFilter('country', 'other')">Otros</button>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="dorasia-card flex bg-gray-900/40 rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 hover:transform hover:scale-[1.01]">
                            ...
                            <div class="dorasia-card__info p-4 flex flex-col flex-grow">
                                ...
                                <div class="dorasia-card__meta flex flex-wrap items-center gap-x-3 gap-y-1 mb-2">
                                    ...
                                    @if($title->country)
                                        <span class="text-sm text-gray-400">{{ $title->country }}</span>
                                    @endif
                                </div>
                                
                                <div class="flex flex-wrap gap-1.5 mb-3">
                                    @foreach($title->genres->take(3) as $g)
                                        <a href="{{ route('catalog.genre', $g->slug) }}" 
                                           class="text-xs bg-gray-800 hover:bg-gray-700 px-2 py-0.5 rounded-full text-gray-300 {{ $g->id == $genre->id ? 'bg-red-900/40 text-red-200' : '' }}">
                                            {{ $g->name }}
                                        </a>
                                    @endforeach
                                </div>
            ...
            @auth
                @php
                    // Esta sección debe ser implementada en el controlador para ser más efectiva
                    ...
                    $watchHistory = $profile ? $profile->watchHistory()->pluck('title_id')->toArray() : [];
                    
                    $personalRecommendations = \App\Models\Title::whereHas('genres', function($query) use ($genre) {
                            $query->where('genres.id', $genre->id);
                        })
                        ->whereNotIn('id', $titles->pluck('id')->toArray())
                        ->where(function($query) use ($watchHistory) {
                            // Priorizar títulos similares a los que el usuario ha visto
                            if (!empty($watchHistory)) {
                                $query->whereIn('category_id', function($subquery) use ($watchHistory) {
                                    $subquery->select('category_id')
                                            ->from('titles')
                                            ->whereIn('id', $watchHistory);
                                });
                            }
                        })
                        ->inRandomOrder()
        ...
        @else
            <div class="bg-gray-900 rounded-lg p-8 text-center my-8">
                ...
                <p class="mt-1 text-gray-400">Aún no hay títulos en este género.</p>
