...
class ImportChineseDramas extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'import:chinese-dramas {--pages=5} {--update} {--force}';
     ...
     * @var string
     */
    protected $description = 'Import Chinese dramas from TMDB API';
    ...
    public function handle()
    {
        ...
        $this->info("Importing Chinese dramas (pages: {$pages}, update: " . ($update ? 'yes' : 'no') . ")");

        // Ensure C-Drama category exists
        $category = Category::firstOrCreate(
            ['slug' => 'c-drama'],
            [
                'name' => 'C-Drama',
                'description' => 'Series de televisión chinas y taiwanesas',
                'language' => 'zh',
                'country' => 'China',
            ]
        );
        ...
        for ($page = 1; $page <= $pages; $page++) {
            ...
            foreach ($dramas as $drama) {
                ...
                try {
                    ...
                    // Determine country specifically for Chinese dramas
                    $country = 'China';
                    if (!empty($details['origin_country'])) {
                        if (in_array('TW', $details['origin_country'])) {
                            $country = 'Taiwán';
                        } elseif (in_array('HK', $details['origin_country'])) {
                            $country = 'Hong Kong';
                        }
                    }
                    ...
                    // Get trailer URL
                    if (!empty($details['videos']) && !empty($details['videos']['results'])) {
                        // Buscar el primer trailer de YouTube
                        foreach ($details['videos']['results'] as $video) {
                            if ($video['site'] === 'YouTube' && ($video['type'] === 'Trailer' || $video['type'] === 'Teaser')) {
                                $titleData['trailer_url'] = 'https://www.youtube.com/watch?v=' . $video['key'];
                                break;
                            }
                        }
                    }
    ...
    protected function importSeasonsAndEpisodes(Title $title, array $details): void
    {
        ...
        foreach ($seasons as $seasonData) {
            ...
            // Get season details directly
            $apiKey = '69e4c68f7f83d0388b2f66bfaca96337';
            $baseUrl = 'https://api.themoviedb.org/3';
            $url = "{$baseUrl}/tv/{$title->tmdb_id}/season/{$season->number}?" . http_build_query([
                'api_key' => $apiKey,
                'language' => 'es-ES'
            ]);
            ...
            if ($httpCode != 200) {
                continue; // Skip this season if we can't get the details
            }
            
            $seasonDetails = json_decode($response, true);
